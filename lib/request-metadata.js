...
function requestMetadata (context, router) {
  ...
  function swaggerApiMetadata (req, res, next) {
    util.log("RequestMetadata --- swaggerApiMetadata");
  ...
  function swaggerPathMetadata (req, res, next) {
    util.log("RequestMetadata --- swaggerPathMetadata");
    ...
    if (req.swagger.api) {
      ...
      Object.keys(req.swagger.api.paths).some(swaggerPath => {
        ...
        if (swaggerPathNormalized === relPathNormalized) {
          ...
          req.swagger.path === null &&
          pathMatches(relPathNormalized, swaggerPathNormalized)
        ) {
      ...
      if (req.swagger.path) {
        ...
        util.warn(
          'WARNING! Unable to find a Swagger path that matches "%s"',
          req.path
        );
      }
    }
...
function swaggerMetadata (req, res, next) {
  util.log("RequestMetadata --- swaggerMetadata");
...
function swaggerOperationMetadata (req, res, next) {
  util.log("RequestMetadata --- swaggerOperationMetadata");

  
  if (req.swagger.path) {
    let method = req.method.toLowerCase();
    console.log(method in req.swagger.path);
    
    if (method in req.swagger.path) {
      console.log('###@@##', req.swagger.path[method].schema);
      if (method !== 'post')
        if (req.swagger.path[method].responses.default.schema.properties.data) {
          const x = req.swagger.path[method].responses.default.schema.properties.data;
          delete req.swagger.path[method].responses.default.schema;
          req.swagger.path[method].responses.default.schema = x;
        }
      req.swagger.operation = req.swagger.path[method];
      // console.log('#####', req.swagger.operation.responses.default.schema);
      console.log('#####', req.swagger.operation.responses);
      
    }
    else {
      util.warn(
        "WARNING! Unable to find a Swagger operation that matches %s %s",
...
function swaggerParamsMetadata (req, res, next) {
  util.log("RequestMetadata --- swaggerParamsMetadata");
  req.swagger.params = util.getParameters(
    req.swagger.path,
...
function getRelativePath (req) {
  util.log("RequestMetadata --- getRelativePath");
...
function pathMatches (path, swaggerPath) {
  util.log("RequestMetadata --- pathMatches");

  // Convert the Swagger path to a RegExp
  let pathPattern = swaggerPath.replace(
    util.swaggerParamRegExp,
    (match, paramName) => {
      return "([^/]+)";
    }
  );
