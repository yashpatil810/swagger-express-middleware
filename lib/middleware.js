...
function Middleware (sharedRouter) {
  ...
  this.init = function (swagger, callback) {
    util.log("Middleware -- Init");

    // the swagger variable should only ever be a string or a populated object.
    let invalidSwagger =
      _.isFunction(swagger) || _.isDate(swagger) || _.isEmpty(swagger);
  ...
  this.files = function (router, options) {
    util.log("Middleware --- Files");

    if (
      arguments.length === 1 &&
      !util.isExpressRouter(router) &&
      !util.isExpressRoutingOptions(router)
    ) {
      // Shift arguments
      options = router;
      router = sharedRouter;
  ...
  this.parseRequest = function (router, options) {
    util.log("Middleware --- ParseRequest");
  ...
  this.validateRequest = function () {
    util.log("Middleware --- ValidateRequest");
  ...
  this.mock = function (router, dataStore) {
    util.log("Middleware --- Mock");

    if (arguments.length === 1 && router instanceof DataStore) {
      // Shift arguments
      dataStore = router;
      router = undefined;
    }
