...
function fileServer (context, router, options) {
  ...
  function serveDereferencedSwaggerFile (req, res, next) {
    util.log("FileServer --- serveDereferencedSwaggerFile");
    ...
    if (req.method === "GET" || req.method === "HEAD") {
      ...
      if (reqPath === configPath) {
        if (context.api) {
          util.debug(
            "%s %s => Sending the Swagger API as JSON",
            req.method,
            req.path
          );
          ...
          util.warn(
            "WARNING! The Swagger API is empty. Sending an HTTP 500 response to %s %s",
            req.method,
            req.path
          );
          res.status(500).json({});
  ...
  function serveRawSwaggerFiles (req, res, next) {
    util.log("FileServer --- serveRawSwaggerFiles");
    ...
    if (req.method === "GET" || req.method === "HEAD") {
      ...
      if (
        _.startsWith(reqPath, configPath) &&
        context.parser &&
        context.parser.$refs
      ) {
        ...
        if (filePath) {
          ...
          util.debug(
            "%s %s does not match any files in the Swagger API",
            req.method,
            req.path
          );
        }
  ...
  function getConfiguredPath (path) {
    util.log("FileServer --- getConfiguredPath");
