...
function requestValidator (context) {
  ...
  function http500 (req, res, next) {
    util.log("RequestValidator --- http500");
...
function http401 (req, res, next) {
  util.log("RequestValidator --- http401");
    ...
    let isValid = req.swagger.security.some(requirement => {
      ...
      return securityDefinitions.every(securityDef => {
        ...
        if (securityDef.type === "basic") {
          ...
          securityDef.type === "apiKey" &&
          securityDef.in === "header"
        ) {
    ...
    if (!isValid) {
      ...
      util.debug(
        "The client didn't provide authentication information for any of the required authentication types (%s). " +
          "Returning HTTP 401 (Unauthorized)",
        securityTypes
      );
      res.set(
        "WWW-Authenticate",
        'Basic realm="' + (req.hostname || "server") + '"'
      );
      throw ono(
        { status: 401 },
        "%s %s requires authentication (%s)",
        req.method,
        req.path,
        securityTypes
      );
...
function http404 (req, res, next) {
  util.log("RequestValidator --- http404");
...
function http405 (req, res, next) {
  util.log("RequestValidator --- http405");
...
function http406 (req, res, next) {
  util.log("RequestValidator --- http406");
  ...
  if (util.isSwaggerRequest(req)) {
    ...
    if (produces.length > 0) {
      ...
      if (!req.accepts(produces)) {
        ...
        throw ono(
          { status: 406 },
          "%s %s cannot produce any of the requested formats (%s). \nSupported formats: %s",
          req.method,
          req.path,
          accepts.join(", "),
          produces.join(", ")
...
function http413 (req, res, next) {
  util.log("RequestValidator --- http413");
  ...
  if (util.isSwaggerRequest(req)) {
    ...
    if (!bodyAllowed) {
      ...
      if (length > 0) {
        ...
        throw ono(
          { status: 413 },
          "%s %s does not allow body content",
          req.method,
          req.path
        );
      }
...
function http415 (req, res, next) {
  util.log("RequestValidator --- http415");
  ...
  if (util.isSwaggerRequest(req)) {
    ...
    if (!_.isEmpty(req.body)) {
      ...
      let consumes =
        req.swagger.operation.consumes || req.swagger.api.consumes || [];

      if (consumes.length > 0) {
        util.debug(
          "Validating Content-Type header (%s)",
        ...
        if (!req.is(consumes)) {
          ...
          throw ono(
            { status: 415 },
            '%s %s does not allow Content-Type "%s". \nAllowed Content-Types: %s',
            req.method,
            req.path,
            contentType,
            consumes.join(", ")
