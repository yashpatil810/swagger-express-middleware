...
exports.warn = function (err, message, params) {
  if (process.env.WARN !== "off") {
    if (_.isString(err)) {
      ...
      console.warn(
        format.apply(null, _.drop(arguments, 1)) + " \n" + err.stack
      );
    }
    else {
...
exports.log = arg => {
  console.log("Function Called  ::: ", arg);
};
/**
...
exports.isExpressApp = function (router) {
  return (
    exports.isExpressRouter(router) &&
...
exports.isExpressRouter = function (router) {
  return _.isFunction(router) && _.isFunction(router.param);
};
...
exports.rfc1123 = function (date) {
  console.log("Function Called --- rfc1123");
  console.log(date);
...
exports.getResponsesBetween = function (operation, min, max) {
  return _.map(operation.responses, (response, responseCode) => {
    return {
    ...
    .filter(response => {
      return (
        (response.code >= min && response.code <= max) ||
        _.isString(response.code)
      );
    });
};
