...
let MAX_VALUE = Number.MAX_VALUE || 1.7976931348623157e308;
...
JsonSchema.prototype.parse = function (value, propPath) {
  util.log("Helper --- JsonSchema.prototype.parse");
...
JsonSchema.prototype.serialize = function (value, propPath) {
  util.log("Helper --- JsonSchema.prototype.serialize");
...
JsonSchema.prototype.sample = function () {
  util.log("Helper --- JsonSchema.prototype.sample");
...
function getValueToValidate (schema, value) {
  // Is the value empty?
  util.log("Helper --- JsonSchema --- getValueToValidate");
...
function jsonValidate (schema, value, propPath) {
  util.log("Helper --- JsonSchema --- jsonValidate");

  if (tv4.validate(value, schema)) {
    return true;
  }
  else if (propPath !== undefined) {
    let dataPath = propPath
      ? propPath + "." + tv4.error.dataPath
      : tv4.error.dataPath;
    throw ono(
      tv4.error,
      { status: 400 },
...
function parseInteger (schema, value, propPath) {
  util.log("Helper --- JsonSchema --- parseInteger");
  ...
  if (
    ...
    throw ono(
      { status: 400 },
      '"%s" is not a properly-formatted whole number',
      propPath || value
    );
  }
...
function serializeInteger (schema, value, propPath) {
  util.log("Helper --- JsonSchema --- serializeInteger");
...
function sampleInteger (schema) {
  util.log("Helper --- JsonSchema --- sampleInteger");
...
function parseNumber (schema, value, propPath) {
  util.log("Helper --- JsonSchema --- parseNumber");
...
function serializeNumber (schema, value, propPath) {
  util.log("Helper --- JsonSchema --- serializeNumber");
...
function sampleNumber (schema) {
  util.log("Helper --- JsonSchema --- sampleNumber");
...
function parseBoolean (schema, value, propPath) {
  util.log("Helper --- JsonSchema --- parseBoolean");
...
function serializeBoolean (schema, value, propPath) {
  util.log("Helper --- JsonSchema --- serializeBoolean");
...
function parseString (schema, value, propPath) {
  util.log("Helper --- JsonSchema --- parseString");
...
function serializeString (schema, value, propPath) {
  util.log("Helper --- JsonSchema --- serializeString");
...
function sampleString (schema) {
  util.log("Helper --- JsonSchema --- sampleString");
...
function parseArray (schema, value, propPath) {
  util.log("Helper --- JsonSchema --- parseArray");
...
function parseArrayItems (schema, array, propPath) {
  util.log("Helper --- JsonSchema --- parseArrayItems");
  ...
  for (let i = 0; i < array.length; i++) {
    ...
    try {
      ...
      throw ono(
        e,
        { status: 400 },
        "Unable to parse %s item at index %d (%j).",
        propPath || "array",
        i,
        item
      );
...
function serializeArray (schema, value, propPath) {
  util.log("Helper --- JsonSchema --- serializeArray");
...
function sampleArray (schema) {
  util.log("Helper --- JsonSchema --- sampleArray");
...
function parseObject (schema, value, propPath) {
  util.log("Helper --- JsonSchema --- parseObject");
  ...
  _.forEach(schema.properties, (prop, propName) => {
    if (parsedValue[propName] !== undefined || prop.default !== undefined) {
      ...
      parsedValue[propName] = propSchema.parse(
        parsedValue[propName],
        fullPropPath
      );
    }
  });
...
function serializeObject (schema, value, propPath) {
  util.log("Helper --- JsonSchema --- serializeObject");
...
function sampleObject (schema) {
  util.log("Helper --- JsonSchema --- sampleObject");
...
function parseFile (schema, value, propPath) {
  util.log("Helper --- JsonSchema --- parseFile");
  ...
  if (schema.minLength) {
    ...
    if (isNaN(minLength)) {
      throw ono(
        { status: 500 },
        'The "minLength" value in the Swagger API is invalid (%j)',
        schema.minLength
      );
    }
    ...
    if (parsedValue.size < minLength) {
      throw ono(
        { status: 400 },
        '%s "%s" is only %d bytes. The minimum is %d bytes',
        propPath || "File",
  ...
  if (schema.maxLength) {
    ...
    if (isNaN(maxLength)) {
      throw ono(
        { status: 500 },
        'The "maxLength" value in the Swagger API is invalid (%j)',
        schema.maxLength
      );
    }
    ...
    if (parsedValue.size > maxLength) {
      throw ono(
        { status: 413 },
        '%s "%s" is %d bytes. The maximum is %d bytes',
        propPath || "File",
        parsedValue.originalname,
        parsedValue.size,
        maxLength
...
function serializeFile (schema, value, propPath) {
  util.log("Helper --- JsonSchema --- serializeFile");
...
function parseDate (schema, value, propPath) {
  util.log("Helper --- JsonSchema --- parseDate");
  ...
  if (_.isDate(value)) {
    ...
    if (!formatPattern.test(value)) {
      throw ono(
        { status: 400 },
        '"%s" is not a properly-formatted %s',
        propPath || value,
        schema.format
      );
    }
  ...
  if (schema.minimum) {
    ...
    if (isNaN(minDate.getTime())) {
      throw ono(
        { status: 500 },
        'The "minimum" value in the Swagger API is invalid (%j)',
        schema.minimum
      );
    }
    ...
    if (schema.exclusiveMinimum === true) {
      if (parsedValue.getTime() === minDate.getTime()) {
        throw ono(
          { status: 400 },
          "%s (%j) is equal to exclusive minimum %j",
          propPath || "Value",
          parsedValue,
          minDate
        );
  ...
  if (schema.maximum) {
    ...
    if (isNaN(maxDate.getTime())) {
      throw ono(
        { status: 500 },
        'The "maximum" value in the Swagger API is invalid (%j)',
        schema.maximum
      );
    }
    ...
    if (schema.exclusiveMaximum === true) {
      if (parsedValue.getTime() === maxDate.getTime()) {
        throw ono(
          { status: 400 },
          "%s (%j) is equal to exclusive maximum %j",
          propPath || "Value",
          parsedValue,
          maxDate
        );
...
function serializeDate (schema, value, propPath) {
  util.log("Helper --- JsonSchema --- serializeDate");
...
function sampleDate (schema) {
  util.log("Helper --- JsonSchema --- sampleDate ");

  let min, max;
  if (schema.minimum !== undefined) {
    min =
      parseInt(new Date(schema.minimum).valueOf()) +
      (schema.exclusiveMinimum ? 1 : 0);
  }
  else {
  ...
  if (schema.maximum !== undefined) {
    max =
      parseInt(new Date(schema.maximum).valueOf()) -
      (schema.exclusiveMaximum ? 1 : 0);
  }
  else {
