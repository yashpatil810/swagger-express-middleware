...
FileDataStore.prototype.__openDataStore = function (collection, callback) {
  util.log("FileDataStore --- FileDataStore.prototype.__openDataStore");

  fs.readFile(
    getFilePath(this.__baseDir, collection),
    { encoding: "utf8" },
    (err, data) => {
      if (err) {
        if (err.code === "ENOENT") {
 ...
 * @protected
 */
FileDataStore.prototype.__saveDataStore = function (
  collection,
  resources,
  callback
) {
  util.log("FileDataStore --- FileDataStore.prototype.__saveDataStore");
  ...
  mkdirp(getDirectory(this.__baseDir, collection), err => {
    if (err) {
      ...
      fs.writeFile(
        getFilePath(self.__baseDir, collection),
...
function getDirectory (baseDir, collection) {
  util.log("FileDataStore --- getDirectory");
...
function getFilePath (baseDir, collection) {
  util.log("FileDataStore --- getFilePath");

  let directory = getDirectory(baseDir, collection);
  let fileName =
    collection.substring(collection.lastIndexOf("/") + 1) + ".json";
