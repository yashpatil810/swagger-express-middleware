...
function SemanticResponse (response, path) {
  util.log("Mock --- SemanticResponse");
...
SemanticResponse.prototype.wrap = function (data) {
  util.log("Mock --- SemanticResponse.prototype.wrap");
  ...
  if (this.isWrapped) {
    ...
    // console.log(wrapper, this.wrapperProperty);
...
SemanticResponse.prototype.setWrapperInfo = function (response, path) {
  util.log("Mock --- SemanticResponse.prototype.setWrapperInfo");

  let self = this;

  // console.log('####response.schema', response.schema);
  // console.log('SETWRAPPER is collection', response.schema.type);
  if (response.schema.type === "array") {
    ...
    response.schema.type === undefined
  ) {
    ...
    if (!schemasMatch(resourceSchemas, response.schema)) {
      ...
      _.some(response.schema.properties, (propSchema, propName) => {
        ...
        // console.log('_.some propeSchema.type', propSchema);
        ...
        if (propSchema.type === "object" || propSchema.type === undefined) {
          ...
          // console.log('###self.schema.properties', self.schema.properties);
...
function getResourceSchemas (path) {
  util.log("Mock --- getResourceSchemas");
...
function schemasMatch (schemasToMatch, schemaToTest) {
  ...
  return schemasToMatch.some(schemaToMatch => {
    ...
    if (
      schemaToTest.type === schemaToMatch.type &&
      propertiesToTest === propertiesToMatch
    ) {
