...
function mergeCollection (req, res, next, dataStore) {
  ...
  res.swagger.location = _.isArray(req.body)
    ? collection
...
function overwriteCollection (req, res, next, dataStore) {
  util.log("MOCK --- overwriteCollection");
...
function createResources (req) {
  ...
  for (let i = 0; i < body.length; i++) {
    ...
    let resourceName = new JsonSchema(propInfo.schema).serialize(
      propInfo.value
    );
...
function getResourceNameByName (data, schema) {
  ...
  let lowercasePropNames = propNames.map(propName => {
