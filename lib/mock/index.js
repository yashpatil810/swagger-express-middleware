...
const MongoDataStore = require("../mongo/mongo-data-store");
/**
...
function mock (context, router, dataStore) {
  ...
  dataStore = dataStore instanceof DataStore ? dataStore : new MongoDataStore();
  console.log("hhhhhhhhhhhhhhh", dataStore);
  ...
  function mockResponse (req, res, next) {
    ...
    if (util.isSwaggerRequest(req) && !isDisabled()) {
      ...
      if (res.statusCode && req.swagger.operation.responses[res.statusCode]) {
        ...
        let responses = util.getResponsesBetween(
  ...
  function mockImplementation (req, res, next) {
    util.log("Mock --- mockImplementation");
    ...
    if (res.swagger) {
      ...
      let db = util.isExpressApp(router)
        ? router.get("mock data store") || dataStore
        : dataStore;
      db.__router = router;
...
function mockResponseHeaders (req, res, next) {
  util.log("Mock --- mockResponseHeaders");

  console.log(res.body);
  console.log(res.swagger);

  if (res.swagger) {
    util.debug(
      "Setting %d response headers...",
      _.keys(res.swagger.headers).length
    );
      ...
      _.forEach(res.swagger.headers, (header, name) => {
        ...
        if (res.get(name) !== undefined) {
          ...
          switch (name.toLowerCase()) {
            ...
            case "last-modified":
              console.log(name);
              console.log(res.swagger.lastModified);
              console.log(util.rfc1123(res.swagger.lastModified));
            ...
            case "content-disposition":
              res.set(
                name,
                'attachment; filename="' +
                  path.basename(res.swagger.location || req.path) +
                  '"'
              );
              break;
            case "set-cookie":
              ...
              if (req.cookies.swagger === undefined) {
                res.cookie(
                  "swagger",
                  _.uniqueId("random") + _.random(99999999999.9999)
                );
              }
              else {
            ...
            default:
              ...
              console.log("deeeeeeeeeeefffffffaaaaaaauuuuuuullllllltttttt");
...
function mockResponseBody (req, res, next) {
  util.log("Mock --- mockResponseBody");
  console.log(res.body);
  ...
  if (res.swagger) {
    if (res.swagger.isEmpty) {
      ...
      util.debug(
        "%s %s does not have a response schema. Sending an empty response",
        req.method,
        req.path
      );
      res.send();
...
function sendText (req, res, next, data) {
  util.log("Mock --- sendText");

  setContentType(
    req,
    res,
    ["text", "html", "text/*", "application/*"], // allow these types
    ["json", "*/json", "+json", "application/octet-stream"]
  ); // don't allow these types
...
function sendFile (req, res, next, file) {
  ...
  fs.exists(file.path || "", exists => {
    if (exists) {
      ...
      util.debug(
        'Unable to find the file: "%s".  Sending an HTTP 410 (Gone)',
        file.path
      );
...
function setContentType (req, res, supported, excluded) {
  util.log("Mock --- setContentType");

  // Get the MIME types that this operation produces
  let produces =
    req.swagger.operation.produces || req.swagger.api.produces || [];

  if (produces.length === 0) {
    // No MIME types were specified, so just use the first one
    util.debug(
      'No "produces" MIME types are specified in the Swagger API, so defaulting to %s',
      supported[0]
    );
    ...
    if (match) {
      util.debug(
        "Using %s MIME type, which is allowed by the Swagger API",
        match
      );
      res.type(match);
    }
